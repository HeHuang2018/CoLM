...
For example, to run the `IMAGENET → IMAGNET-C` benchmark with the `CoLM` adapter, execute:

```bash
python L-CS.py -acfg configs/adapter/imagenet/CoLM.yaml -dcfg configs/dataset/imagenet.yaml -ocfg configs/order/imagenet/0.yaml SEED 0
```
